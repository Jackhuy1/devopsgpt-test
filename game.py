...
class Snake:
    ...
    def move(self):
        ...
        dx, dy = DIRECTION_OFFSETS.get(self.direction, (0, 0))
        self.head_x += dx
        self.head_y += dy

        # If wrap-around is enabled, wrap the snake's head around the window
        if WRAP_AROUND:
    ...
    def check_collision(self):
        # If wrap-around is disabled, check window boundaries
        if not WRAP_AROUND:
